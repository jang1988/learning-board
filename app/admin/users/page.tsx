  ...
  const { data: employees } = await supabase
    ...
    .select(`
      *,
      ...
      quiz_results(percent, passed)
    `)
    .eq('role', 'manager')
    .order('created_at', { ascending: false })

  const { count: topicsTotal } = await supabase
    .from('topics').select('*', { count: 'exact', head: true })
    ...
    <div className={styles.page}>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          ...
          <tbody>
            {employees?.map(emp => {
              const done = emp.topic_progress?.filter((p: any) => p.status === 'completed').length ?? 0
              ...
              return (
                <tr key={emp.id}>
                  ...
                  <td>
                    <span className={`badge ${done === total && total > 0 ? 'badge--green' : 'badge--gray'}`}>
                      {done} / {total}
                    </span>
                  </td>
                  <td>
                    {avgScore !== null ? (
                      <span className={`${styles.score} ${avgScore >= 70 ? styles.good : styles.bad}`}>
                        {avgScore}%
                      </span>
                    ) : <span className={styles.noData}>—</span>}
                  </td>
                  <td>
                    <RoleToggle userId={emp.id} currentRole={emp.role} />
                  </td>
                </tr>
              )
            })}
